...
<section class="mt-4 space-y-4">
  ...
  {#if registryNodes.length === 0 && registrySenders.length === 0 && registryReceivers.length === 0}
    <div class="rounded-xl border border-slate-800 bg-slate-950/70 p-4 text-[11px] text-slate-300">
      <p class="font-semibold">Registry is empty.</p>
      <p class="mt-1">
        Run an NMOS discovery from the <span class="font-semibold">NMOS</span> tab to ingest nodes and endpoints into the
        internal registry.
      </p>
    </div>
  {:else}
